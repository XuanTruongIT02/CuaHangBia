...
{
    public class User_62131086Controller : Controller
    {
        ...
        [HttpPost]
        public ActionResult Dangky(Nguoidung_62131086 nguoidung)
        {
            try
            {
        ...
        public ActionResult Dangnhap(FormCollection userlog)
        {
            ...
            if (islogin != null)
            {
                var maquyen = db.Nguoidungs.SingleOrDefault(x => x.Email.Equals(userMail)).IDQuyen;
                if (maquyen == 1)
                {
                    Session["use"] = islogin;
